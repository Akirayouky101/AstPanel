...
window.TasksAPI = {
    ...
    async getAll() {
        const { data, error } = await supabase
            ...
            .select(`
                *,
                ...
                task_preventivi(preventivo_id, preventivi(id, numero, oggetto, cliente_nome))
            `)
        ...
        return data.map(task => ({
            ...
            // Preventivi collegati (multi)
            preventivi_collegati: (task.task_preventivi || []).map(r => r.preventivi).filter(Boolean)
        }));
    },
